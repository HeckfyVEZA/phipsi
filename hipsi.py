...
if table:
    ...
    fig.update_layout(title='График', xaxis_title='ϕ', yaxis_title='ψs', showlegend=True, width=600, height=500, legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99), font=dict(family="Arial", size=18, color="black"), autosize=False, margin=dict(l=50, r=50, b=100, t=100, pad=4), paper_bgcolor="white", plot_bgcolor="white", hovermode="x unified", hoverlabel=dict(bgcolor="white", font_color="black"), title_x=0.5, title_font=dict(family="Arial", size=24, color="black"), title_font_color="black", title_xanchor="center", title_yanchor="top", title_y=0.99, xaxis=dict(showgrid=True, gridcolor='grey', gridwidth=1, tickmode='linear', dtick=0.0125), yaxis=dict(showgrid=True, gridcolor='grey', gridwidth=1, tickmode='linear', dtick=0.1))
    st.plotly_chart(fig)