...
if table:
    ...
    fig.update_layout(xaxis_title='ϕ', yaxis_title='ψs', showlegend=True, width=750, height=500, legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99), hovermode="x unified", hoverlabel=dict(bgcolor="white", font_color="black"), title_x=0.5, title_xanchor="center", title_yanchor="top", title_y=0.99, xaxis=dict(showgrid=True, gridwidth=.5, dtick=0.0125), yaxis=dict(showgrid=True, gridwidth=.5, dtick=0.1))
    st.plotly_chart(fig)